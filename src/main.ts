...
document.addEventListener('DOMContentLoaded', function () {
  tableBody = (document.getElementById('crossword') as Element)
    .children[0] as HTMLTableSectionElement
  ...
  (document.getElementById(
    'saveCrossword'
  ) as HTMLButtonElement).onclick = saveGrid
  ...
  crossword
    ...
    .forEach(function (rowData) {
      ...
      rowData.forEach(function (cellData) {
        ...
        if (cellData.letter !== '') {
          cell.className = `letter word${
            cellData.wordNumber
            } num${cellData.ids.join(' num')}`
  ...
  for (const letter of document.getElementsByClassName('letter')) {
    ...
    for (const otherLetter of document.getElementsByClassName('letter')) {
      for (const cls of classes) {
        if (
          otherLetter.classList.contains(cls) &&
          otherLetter.tagName.toLowerCase() === 'td'
        ) {
...
// Adds the "Delete word XXXXX" paragraph to the list
function addDeleteWord() {
  for (const child of wordList.children) {
    const cls = child.className
      .split(' ')
      .filter(cls => cls.startsWith('num'))[0]
    // @ts-ignore
    const classNum = parseInt(cls.match(/num(\d+)/)[1])
    const trash = Array.from(child.children).filter(el =>
      el.classList.contains('deleteWord')
...
function saveGrid() {
  ...
  let realSize =
    verticalSize < horizontalSize ? cell.offsetHeight : cell.offsetWidth
